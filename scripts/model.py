...
class MultimodalArchitecture(torch.nn.Module):
	def __init__(self, device, CNN_TO_USE = 'resnet34', MODALITY = 'multimodal', MIL_architecture = 'ADMIL', N_CLASSES = 5, fc_input_features = 512, hidden_space_len=128, TEMPERATURE = 0.07):
		"""
		In the constructor we instantiate two nn.Linear modules and assign them as
		member variables.
		"""
		...
		if (self.MODALITY is not MOD.unimodal or self.MIL_architecture is ALG.ADMIL):
		...
		# parameters bert
		#if (self.MODALITY is not MOD.unimodal):
		...
		if (self.MODALITY is MOD.multimodal_clip):
		...
		elif (self.MODALITY is MOD.multimodal or self.MIL_architecture is ALG.ADMIL):
			self.intermediate_embedding_1 = torch.nn.Linear(in_features=self.O, out_features=self.I)
	...
	def forward(self, input_img, input_txt, labels, phase):
		...
		if input_img is not None:
			...
			if (self.MIL_architecture is ALG.CLAM_SB):
			...
			if (self.MODALITY is MOD.multimodal_clip):
				...
				output_img = self.intermediate_embedding_1(output_img)
		...
		if input_txt is not None:
			...
			if (self.MODALITY is MOD.multimodal_clip):
				...
				output_txt = self.intermediate_embedding_1(intermediate_txt)
				#output_txt = self.activation(output_txt)
				output_txt = output_txt.view(-1)
