...
from enum_multi import TYPE, AGGREGATION, METRIC
...
parser.add_argument('-t', '--metric', help='precision_at/mAP',type=str, default='precision_at')
parser.add_argument('-c', '--aggregation', help='normal/micro',type=str, default='micro')
...
if (AGGREGATION_mode is AGGREGATION.normal and PERCENTAGE == -1):
	filename_output = retrieval_path + metric_str + '_'+str(at_k)+'_'+DATASET+'_'+type_data+'.csv'

elif (AGGREGATION_mode is AGGREGATION.normal and at_k == -1):
	filename_output = retrieval_path + metric_str + '_percentage_'+str(PERCENTAGE)+'_'+DATASET+'_'+type_data+'.csv'

elif (AGGREGATION_mode is AGGREGATION.micro and PERCENTAGE == -1):
	filename_output = retrieval_path + metric_str + '_micro_'+str(at_k)+'_'+DATASET+'_'+type_data+'.csv'

elif (AGGREGATION_mode is AGGREGATION.micro and at_k == -1):
	filename_output = retrieval_path + metric_str + '_micro_percentage_'+str(PERCENTAGE)+'_'+DATASET+'_'+type_data+'.csv'
...
def get_precision(DATASET, test_dataset):
	...
	for wsi in test_dataset:
		...
		if (AGGREGATION_mode is AGGREGATION.normal and metric is METRIC.precision_at):
			re = utils_retrieval.relevant(label, labels_found)
			
			p = re / num_elem_to_retrieve
			r = utils_retrieval.recall(label, re, CONT_CLASSES)
			#print("id:" + str(fname) + ", precision: " + str(p), labels_found, label)
			precisions.append(p)

		elif (AGGREGATION_mode is AGGREGATION.micro and metric is METRIC.precision_at):

			re, tot_to_retrieve = utils_retrieval.relevant_micro(label, labels_found)
	
			p = re / tot_to_retrieve
			#print("id:" + str(fname) + ", precision: " + str(p))
			precisions.append(p)

		elif (AGGREGATION_mode is AGGREGATION.normal and metric is METRIC.mAP):
			cont = 0
			...
			cont = cont + 1
			"""
			re = relevant(label, labels_found)
			
			p = re / num_elem_to_retrieve
			r = recall(label, re)
			"""
			
		elif (AGGREGATION_mode is AGGREGATION.micro and metric is METRIC.mAP):
	...
	if (metric is METRIC.precision_at):
		tot_precision = np.sum(precisions) / len(precisions)
		print("precision " + str(DATASET) + ": " + str(tot_precision))
