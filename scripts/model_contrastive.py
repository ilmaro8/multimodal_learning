...
class MultimodalArchitecture(torch.nn.Module):
	def __init__(self, device, CNN_TO_USE = 'resnet34', MODALITY = 'multimodal', MIL_architecture = 'ADMIL', N_CLASSES = 5, fc_input_features = 512, hidden_space_len=128, TEMPERATURE = 0.07):
		...
		if (self.MIL_architecture is ALG.ADMIL):
		...
		if (self.MODALITY is MOD.contrastive_clip):
		...
		elif (self.MODALITY is MOD.contrastive):
	...
	def forward(self, input_img, input_txt, labels, phase):
		...
		if input_img is not None:
			...
			elif (self.MODALITY is MOD.contrastive or self.MIL_architecture is ALG.ADMIL):
